...
def normalize(in_str: str) -> str:
    ...
    for locSymb in return_sr_start:
        if locSymb in string.ascii_letters or locSymb in string.digits:
            return_str += locSymb
        else:
            return_str += "_"
...
def file_rename_with_control(in_file: Path, in_new_short_file_name: str, iter_suffix: int = 0):
    if iter_suffix == 0:
        new_file = Path(in_file.parent.as_posix() + "/" + in_new_short_file_name + in_file.suffix)
    else:
        new_file = Path(
            in_file.parent.as_posix() + "/" + in_new_short_file_name + "_" + str(iter_suffix) + in_file.suffix)

    if not new_file.exists():
        in_file.rename(new_file)
    else:
        iter_suffix += 1
        file_rename_with_control(in_file, in_new_short_file_name, iter_suffix)
...
def normalize_names_in_folders(in_folder: Path):
    ...
    for internal_object in in_folder.iterdir():
        if internal_object.is_dir():
            ...
            internal_object.rename(internal_object.with_name(normalize(internal_object.name)))
        else:
            file_rename_with_control(internal_object, normalize(internal_object.stem))
    return 0


def file_move_into_with_control(in_file: Path, in_move_folder: Path, iter_suffix: int = 0):
    if iter_suffix == 0:
        new_file = Path(in_move_folder.as_posix() + "/" + in_file.name)
    else:
        new_file = Path(in_move_folder.as_posix() + "/" + in_file.stem + "_" + str(iter_suffix) + in_file.suffix)
...
def sort_files(in_folder: Path):
    ...
    for file_extension_folder, file_extension_list in file_extension_dict.items():
...
def init_global_variables(in_litter_folder: str):
    ...
    file_extension_dict = {Path(in_litter_folder + "\\" + ext_folder): ext_list for ext_folder, ext_list in
